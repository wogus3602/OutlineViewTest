...
final class AdjustableSeparatorRowView: NSTableRowView {
    ...
    static let setupSwizzling: Void = {
        ...
        let originalImplementation = method_setImplementation(originalMethod, method_getImplementation(newMethod))
    ...
    @objc func separatorRect() -> CGRect {
        ...
        guard let view = self.view(atColumn: 0) as? NSView else {
            return .zero
        }
