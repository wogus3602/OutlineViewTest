...
struct OutlineViewUpdater<Data: Sequence> where Data.Element: Identifiable {
    ...
    func performUpdates(
        ...
        let diff = newNonOptionalState.difference(from: oldNonOptionalState, by: { $0.value.id == $1.value.id })
        ...
        for change in diff {
            ...
            case .insert(offset: let offset, _, _):
                outlineView.insertItems(at: IndexSet([offset]), inParent: parent, withAnimation: .effectFade)

            case let .remove(offset: offset, element: element, _):
                removedElements.append(element)
                outlineView.removeItems(at: IndexSet([offset]), inParent: parent, withAnimation: .effectFade)
            }
        }
        ...
        oldUnchangedElements
            .keys
            .map { (oldUnchangedElements[$0], newStateDict[$0]) }
